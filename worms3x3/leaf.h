...
constexpr uint32_t HAS_HEAD_MASK = 1u<<30;
constexpr uint32_t IS_LEAF_MASK = 1u<<31;
...
struct Leaf2x2 {
    ...
    Leaf2x2(uint32_t index) {
        ...
        endy = (index >> 25) & 1;
        dir = (index >> 26) & 7;
        assert(hasEnd() == bool(index&HAS_HEAD_MASK));
    }
    ...
    static int edgesCount(uint32_t index) {
        assert(index & IS_LEAF_MASK);
        return popcount(index & ((1u<<24)-1));
    }
};
